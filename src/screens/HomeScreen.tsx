...
import { View, Text, StyleSheet, Image, ScrollView, ActivityIndicator, Pressable, Modal, Alert } from "react-native";
  ...
  useEffect(() => {
    ...
    const loadMoments = async () => {
      ...
      const { data, error } = await supabase
        ...
        .limit(20);
      if (!error && data) {
        setMoments(data.filter((s: any) => !!s.image_url).map((s: any) => ({ id: s.id, image_url: s.image_url })));
      }
  ...
  async function handleAddMoment() {
    try {
      ...
      const { data } = await supabase
        ...
        .limit(20);
      if (data) {
        setMoments((data as any[]).filter(s => !!s.image_url).map(s => ({ id: s.id, image_url: s.image_url! })));
      }
    } catch (e) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }] }>
      ...
      <View style={styles.header}>
        <Text style={[styles.logo, { color: theme.headerText }]}>Whirl</Text>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        {/* CAMPUS MOMENTS (blank slate) */}
        <View style={[styles.sectionBoxBlue, { borderColor: theme.accent }] }>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={[styles.sectionTitle, { color: theme.textPrimary }]}>Campus Moments</Text>
            <Pressable onPress={handleAddMoment} style={[styles.addBtn, { borderColor: theme.accent }]}>
              <Text style={{ color: theme.accent, fontFamily: 'CherryBomb' }}>Add</Text>
            </Pressable>
          </View>
          {momentsLoading ? (
            <View style={styles.blankSlate}><ActivityIndicator /></View>
          ) : moments.length === 0 ? (
            <View style={styles.blankSlate}>
              <Text style={[styles.blankSlateText, { color: theme.textSecondary }]}>No moments yet</Text>
            </View>
          ) : (
        ...
        <View style={[styles.sectionBoxGray, { backgroundColor: theme.card }] }>
          <Text style={[styles.sectionTitle, { color: theme.textPrimary }]}>What's going on</Text>
          {loading && (
          ...
          {!loading && !errorMsg && posts.map((p) => (
            <View key={p.id} style={{ marginBottom: 8 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                {/* Left: user + username + date + caption */}
                <View style={{ flex: 1, paddingRight: 8 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
                    <Pressable onPress={() => p.author?.id && (navigation as any).navigate('UserProfile', { userId: p.author.id })}>
                      {p.author?.avatar_url ? (
                        <Image source={{ uri: p.author.avatar_url }} style={styles.postAvatar} />
                      ) : (
                        <Image source={require('../../assets/avatars/Default_pfp.jpg')} style={styles.postAvatar} />
                      )}
                    </Pressable>
                    <Pressable onPress={() => p.author?.id && (navigation as any).navigate('UserProfile', { userId: p.author.id })}>
                      <Text style={{ fontWeight: '700', fontSize: 13, marginLeft: 6, fontFamily: 'CherryBomb', color: theme.textPrimary }} numberOfLines={1}>
                  ...
                  {p.caption && <Text style={{ marginTop: 4, fontSize: 13, color: theme.textPrimary }}>{p.caption}</Text>}
                </View>
                ...
                >
                  {p.image_url ? (
                    <Image
                      ...
                      style={{ width: '100%', height: 100, borderRadius: 8, backgroundColor: '#fff' }}
                      resizeMode="contain"
                    />
                  ) : null}
                </Pressable>
        ...
        {/* 24-Hour Moments removed */}
      </ScrollView>
      {/* Taskbar removed */}
      {/* Photo modal */}
      <Modal visible={photoModalVisible} transparent animationType="fade" onRequestClose={() => setPhotoModalVisible(false)}>
        <View style={styles.modalBackdrop}>
          <Pressable style={StyleSheet.absoluteFill} onPress={() => setPhotoModalVisible(false)} />
          {photoModalUri ? (
            <Image source={{ uri: photoModalUri }} style={styles.modalImage} resizeMode="contain" />
          ) : (
            <View style={{ alignItems: 'center' }}>
              <Text style={{ color: '#fff' }}>No image to show</Text>
            </View>
          )}
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  logo: { fontSize: 36, fontWeight: "800", fontFamily: 'CherryBomb' },
  ...
  sectionTitle: { fontSize: 18, fontWeight: "700", fontFamily: 'CherryBomb' },
  momentsRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginTop: 10,
  ...
  addBtn: { paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderRadius: 999 },
  momentLabel: { marginTop: 4, fontWeight: "600" },
  blankSlate: {
    marginTop: 10,
    borderWidth: 1,
    borderColor: '#ddd',
  ...
  postRow: { flexDirection: "row", alignItems: "center", marginBottom: 14 },
  postAvatar: { width: 28, height: 28, borderRadius: 14, marginRight: 6 },
  postText: { flex: 1, fontSize: 15, fontWeight: "600" },
  bigImage: { width: 100, height: 180, borderRadius: 12, marginTop: 10 }
