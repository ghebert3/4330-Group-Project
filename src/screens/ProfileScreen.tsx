...
export default function ProfileScreen() {
  ...
  async function loadProfile(isRefresh = false) {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("profiles")
        .select(
          "display_name, bio, tags, looking_for, avatar_url, hometown"
        )
        .eq("id", user.id)
        .single();
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      await Promise.all([loadProfile(true), loadPhotos(true)]);
    } finally {
      setRefreshing(false);
    }
  };
    ...
    <FadeInView style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={styles.photoRow}>
          {photos.map((p, i) => (
            <FadeInView key={p.id ?? i} delay={220 + i * 60}>
              <Pressable onPress={() => openPhoto(i)}>
                  ...
                  return (
                    ...
                    >
                      <Text
                        style={{
                          fontFamily: "CherryBomb",
                          color: selected ? theme.card : theme.textSecondary,
                        }}
                      >
                        {label}
                      </Text>
        ...
        <Modal visible={tagModal} transparent animationType="fade">
          <View style={styles.modalCenter}>
            <View style={[styles.modalBox, { maxHeight: "75%" }]}>
              ...
              >
                Tap to select or deselect. These help people see what you’re
                into.
              </Text>
              ...
              <View style={styles.modalBtnRow}>
                ...
                <Pressable style={styles.modalBtn} onPress={saveTagsToSupabase}>
                  <Text style={styles.modalBtnText}>Save</Text>
                </Pressable>
              </View>
            </View>
